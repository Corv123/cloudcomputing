...
│   └── related_articles_analyzer.py # Related articles (Chart 6)
│
...
**Type:** Article Cards
**Shows:** Up to 8 related articles from credible news sources
**Purpose:** Cross-reference with reputable reporting on the same topic

**Features:**
- Searches Google News RSS + DuckDuckGo (FREE - no API key!)
- Filters to reliable sources only (Reuters, AP, BBC, NYT, etc.)
- Shows relevance scores using TF-IDF + cosine similarity
- All article links are REAL and working (no 404 errors)
- Automatic fallback: Google News → DuckDuckGo
...
# Optional: NewsAPI key for article extraction (not required for Chart 6)
# Chart 6 uses free Google News RSS + DuckDuckGo - no API key needed!
# NEWSAPI_KEY = "your_key_here"  # Uncomment only if you want NewsAPI for extraction
```
...
numpy==1.26.3
lxml==5.1.0
feedparser==6.0.11        # For Google News RSS parsing (Chart 6)
duckduckgo-search==5.0.0  # For DuckDuckGo news fallback (Chart 6)
```
