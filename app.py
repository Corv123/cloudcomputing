...
def analyze_article():
    ...
    try:
        ...
        # Get database article count for dynamic weighting
        db_article_count = len(database_articles)

        # Get dynamic weights based on database size
        weights = config.get_dynamic_weights(db_article_count)

        print(f"📊 Dynamic Weights (DB size: {db_article_count}): Language={weights['language']:.3f}, Credibility={weights['credibility']:.3f}, Crosscheck={weights['crosscheck']:.3f}")

        # Calculate overall score using dynamic weights
        # Note: All scores are on 0-1 scale
        overall_score = (
        ...
        article_data.update({
            ...
            # Transparency data for dynamic scoring
            'score_weights_used': weights,
            'database_article_count': db_article_count,
            'crosscheck_metrics': crosscheck_result.get('metrics', {})
        })
