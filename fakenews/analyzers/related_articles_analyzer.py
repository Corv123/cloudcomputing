...
class RelatedArticlesAnalyzer:
                ...
                try:
                    ...
                    for entry in feed.entries[:40]:
                        try:
                            ...
                            # Clean title to remove HTML entities
                            clean_title = self._clean_html(entry.title)
            ...
            with DDGS() as ddgs:
                ...
                for result in results:
                    try:
                        ...
                        # Validate URL points to an article
                        ...
                        # Clean title to remove HTML entities
                        clean_title = self._clean_html(result['title'])
                        clean_snippet = self._clean_html(result.get('body', ''))
    ...
    def _is_valid_article_url(self, url: str) -> bool:
        """
        Check if URL points to an actual article, not homepage or search result.

        Valid article URLs typically:
        - Have a path longer than just domain (not homepage)
        - Contain article indicators in path
        - Are not search/category pages
        """
        try:
            parsed = urlparse(url)
            path = parsed.path

            # Reject homepage URLs (just domain with / or no path)
            ...
            # Article URLs usually have these indicators
            article_indicators = [
                '/article/', '/news/', '/story/', '/post/',
                '/20', '/2024/', '/2025/',  # Year in URL
                '-'  # Slugs typically have hyphens
            ]

            has_indicator = any(ind in path for ind in article_indicators)

            # Also check path has reasonable length (not just /abc)
            path_length_ok = len(path) > 10

            return has_indicator or path_length_ok

        except:
            return True  # If parsing fails, allow it through
